...
def format_lots(s,u):
    return "0.001 Lots (Min) - Risk $20" if any(x in s for x in ["R_","BOOM","CRASH"]) else f"{max(u/100000,0.01):.2f} Lots"
def calculate_position_size(b,r,e,sl):
    risk=abs(e-sl); return round((b*r)/risk,4) if risk!=0 else 0
def check_circuit_breaker():
    today=datetime.now(timezone.utc).date()
    if daily_stats["date"]!=today:
        daily_stats["date"]=today; daily_stats["losses_today"]=0.0; daily_stats["is_circuit_broken"]=False
    if daily_stats["losses_today"] >= (ACCOUNT_BALANCE*MAX_DAILY_LOSS_PCT):
        daily_stats["is_circuit_broken"]=True; return False
    return True
...
def fetch_deriv_data(symbol,timeframe,limit=100):
    try:
        ...
        deriv_symbol="BOOM_1000" if "BOOM1000" in symbol else "CRASH_1000" if "CRASH1000" in symbol else symbol
        ws=websocket.create_connection("wss://ws.derivws.com/websockets/v3?app_id=1089",timeout=10)
        ws.send(json.dumps({"ticks_history":deriv_symbol,"adjust_start_time":1,"count":limit,"end":"latest","granularity":gran_map.get(timeframe,300),"style":"candles"}))
        res=json.loads(ws.recv()); ws.close()
        if "candles" not in res: return None
        df=pd.DataFrame(res["candles"])
        df['timestamp']=pd.to_datetime(df['epoch'],unit='s',utc=True); df.set_index('timestamp',inplace=True)
        df=df[['open','high','low','close']].astype(float); df['volume']=1000; return df
    except Exception as e: print(f"Deriv {symbol} {e}"); return None

def fetch_data(symbol,tf,limit=100):
    if any(x in symbol for x in ["R_","BOOM","CRASH"]): return fetch_deriv_data(symbol,tf,limit)
    try:
        bars=exchange.fetch_ohlcv(symbol,timeframe=tf,limit=limit)
        df=pd.DataFrame(bars,columns=['timestamp','open','high','low','close','volume'])
        df['timestamp']=pd.to_datetime(df['timestamp'],unit='ms',utc=True); df.set_index('timestamp',inplace=True); return df
    except: return None
...
def fetch_current_price(symbol):
    if any(x in symbol for x in ["R_","BOOM","CRASH"]):
        try:
            import websocket
            d="BOOM_1000" if "BOOM1000" in symbol else "CRASH_1000" if "CRASH1000" in symbol else symbol
        ...
        df=fetch_deriv_data(symbol,'1m',2); return df['close'].iloc[-1] if df is not None else None
...
def analyze_structure(df,window=20):
    ...
    ema=df['close'].ewm(span=20).mean().iloc[-1]; return "BULLISH" if curr>ema else "BEARISH"

def detect_price_action(df):
    if df is None or len(df)<3: return None
    c1=df.iloc[-2]; body=abs(c1['close']-c1['open']); uw=c1['high']-max(c1['close'],c1['open']); lw=min(c1['close'],c1['open'])-c1['low']
...
def evaluate_aplus_setup(symbol):
    ...
    # V11: BUY ONLY V75 on WEEKEND to avoid those SL hits you just got
    if "R_75" in symbol and datetime.now(timezone.utc).weekday()>=5 and "BEARISH" in m5_pa:
        return None
    price=m5['close'].iloc[-1]
    atr=(m5['high']-m5['low']).rolling(14).mean().iloc[-1]
    if pd.isna(atr) or atr==0: atr=price*0.001
    if "R_75" in symbol:
        min_sl=price*0.003; mult=5.0
    elif any(x in symbol for x in ["R_","BOOM","CRASH"]):
        min_sl=price*0.002; mult=3.0
    elif "XAU" in symbol:
        min_sl=2.5; mult=1.8
    else:
        min_sl=atr*1.2; mult=1.2
    if "R_75" in symbol or "R_" in symbol or "BOOM" in symbol or "CRASH" in symbol or "XAU" in symbol:
        sl=(m5['low'].iloc[-3:].min()-max(min_sl,atr*mult)) if "BULLISH" in m5_pa else (m5['high'].iloc[-3:].max()+max(min_sl,atr*mult))
    else:
        sl=(m5['low'].iloc[-3:].min()-atr*mult) if "BULLISH" in m5_pa else (m5['high'].iloc[-3:].max()+atr*mult)
    risk=abs(price-sl)
    ...
    while True:
        try:
            ...
            for trade in list(active_trades):
                ...
                if trade['bias']=="BUY":
                    if not trade['tp1_hit'] and cur>=trade['tp1']:
                        trade['tp1_hit']=True; daily_stats['tp1_hits']+=1
                        ...
                        if not trade['tp1_hit']: daily_stats['losses']+=1; daily_stats['losses_today']+=ACCOUNT_BALANCE*RISK_PER_TRADE_PCT
...
async def market_scanner(app):
    ...
    while True:
        try:
            if is_paused(): print("Paused"); await asyncio.sleep(60); continue
    ...
    while True:
        try:
            ...
            if now.hour==20 and now.minute<=10 and sent!=now.date():
                syms=get_active_symbols(); total=daily_stats['wins']+daily_stats['losses']+daily_stats['tp1_hits']; rw=daily_stats['wins']+daily_stats['tp1_hits']; wr=rw/total*100 if total>0 else 0
...
async def start_command(update,context):
    ...
    await update.message.reply_text(f"StarFx V11 {status} BUY-only V75 weekend\nCooldown 1H no dup\nActive: {', '.join(get_active_symbols())}\n/signal /price /report")
